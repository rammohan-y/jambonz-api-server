...
const {columnExists} = require('../lib/routes/api/utils');
...
test('add an admin user', (t) => {
  if (!columnExists('users', 'is_read_only')) {
    t.pass('skipping test as read-only feature is not enabled');
    t.end();
    return;
  }
...
test('read-only user tests', async(t) => {
  if (!await columnExists('users', 'is_read_only')) {
    t.pass('skipping test as read-only feature is not enabled');
    t.end();
    return;
  }
  ...
  try {
    ...
    t.ok(result.statusCode === 201 && result.body.user_sid, 'service_provider scope read-only user created');
    ...
    t.ok(result.statusCode === 200 && result.body.token, 'successfully logged in as service provider read-only user');
    ...
    try {
      ...
      t.ok(err.statusCode === 403, 'As a read-only user, you cannot create an account');
    }
    ...
    /* add account scope read-only user */
    result = await request.post(`/Users`, {
      resolveWithFullResponse: true,
      json: true,
      auth: authAdmin,
    ...
    t.ok(result.statusCode === 200 && result.body.token, 'successfully logged in as account read-only user');
    let userToken = {bearer: result.body.token};


    /* add an application which should fail as the logged in user is a read-only user */
    try {
      ...
      t.ok(err.statusCode === 403, 'As a read-only user, you cannot create an application');
    }
    ...
    // change user back to read-only and query the application - it should succeed
    result = await request.put(`/Users/${account_user_sid}`, {
      resolveWithFullResponse: true,
      json: true,
      auth: authAdmin,
    ...
    t.ok(result.statusCode === 204, 'successfully updated user permission to read-only');
    ...
    t.ok(result.length === 1 , 'successfully queried all applications with read-only user');
  } catch (err) {
      console.error(err);
      t.end(err);
    }
