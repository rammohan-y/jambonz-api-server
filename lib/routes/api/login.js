...
const {columnExists} = require('./utils');
...
router.post('/', async(req, res) => {
  ...
  try {
    ...
    const isReadOnlyColumnExists = await columnExists('users',  'is_read_only');
    obj.read_only_feature = isReadOnlyColumnExists;
    obj.is_read_only = false;
    if (isReadOnlyColumnExists) {
      obj.is_read_only = r[0].is_read_only;
    }
    ...
    const payload = {
      ...
      read_only_feature: obj.read_only_feature
    };
